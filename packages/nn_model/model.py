...
class Model:
    def __init__(self, weight_file_path: str):
        ...
        # Updated to use ultralytics YOLO instead of torch.hub
        try:
            self.model = YOLO(weight_file_path)
            print(f"Successfully loaded YOLOv11 model from {weight_file_path}")
        except Exception as e:
            print(f"Failed to load YOLOv11 model: {e}")
            print("Falling back to torch.hub YOLOv5 loading...")
            # Fallback to old method if ultralytics fails
    ...
    def infer(self, image: np.ndarray) -> Tuple[list, list, list]:
        # Check if using ultralytics YOLO or old torch model
        if hasattr(self.model, 'predict'):
            # Using ultralytics YOLO
            results = self.model.predict(image, imgsz=IMAGE_SIZE, verbose=False)
            
            if len(results) > 0 and results[0].boxes is not None:
                boxes = results[0].boxes
                xyxy = boxes.xyxy.cpu().numpy()  # Convert to numpy
                conf = boxes.conf.cpu().numpy()
                clas = boxes.cls.cpu().numpy()
                
                return xyxy.tolist(), clas.tolist(), conf.tolist()
            return [], [], []
        else:
            # Using old torch.hub model
            det = self.model(image, size=IMAGE_SIZE)
            xyxy = det.xyxy[0]  # grabs det of first image
